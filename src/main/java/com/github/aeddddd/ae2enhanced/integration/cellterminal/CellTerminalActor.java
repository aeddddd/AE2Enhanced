 ...
 * Cell Terminal 的分区编辑分发路径(容器动作 / 网络批量工具)不携带玩家上下文,
 * 由条件 Mixin 在入口方法处通过本类记录当前执行者, {@link EMCInterfaceFilterHost}
 * 据此执行 {@code TileEMCInterface.canManage} 权限校验. 本类不引用任何
 * cellterminal 类, 可安全无条件加载.
 */
