...
/**
 * 微型奇点仪式配方（JSON 数据驱动,KJS 可通过标准配方管理器增删）.
 * 以目标方块为中心,扫描周围 5×5×5 区域内的物品实体,累加数量后匹配输入.
 * 输入支持三类：丢在世界中的物品、右键手持物品、右键目标方块.
 * 输出为微型奇点方块,可配置存在时间.
 */
public class SingularityRitualRecipe implements Recipe<Container> {
    ...
    public SingularityRitualRecipe(ResourceLocation id, List<ItemStack> droppedInputs,
     ...
     */
    public void craft(Level level, BlockPos center) {
        // 消耗 droppedInputs
        AABB area = new AABB(
                center.getX() - 2, center.getY() - 2, center.getZ() - 2,
                center.getX() + 3, center.getY() + 3, center.getZ() + 3);
    ...
    public RecipeSerializer<?> getSerializer() {
        return ModRecipes.SINGULARITY_RITUAL_SERIALIZER.get();
    }

    @Override
    public RecipeType<?> getType() {
        return ModRecipes.SINGULARITY_RITUAL_TYPE.get();
    }
