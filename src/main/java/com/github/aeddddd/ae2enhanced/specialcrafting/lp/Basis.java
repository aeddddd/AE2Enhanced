...
/**
 * 基矩阵因子化: 稀疏 LU(部分主元)+ Forrest–Tomlin 秩一更新, 布局为 B = P·L·U·E₁·E₂···E_k.
 * 位置(position)是消元顺序的逻辑行号, 矩阵行(row)是原始约束行号, 两者经 rowOfPos / posOfRow 随选主元交换.
 * 数值策略: 主元取剩余位置中模最大者, 低于 {@link #PIVOT_MIN} 判数值失败; eta 数超 {@link #MAX_ETA} 由调用方触发重分解.
 */
final class Basis {

    /** 主元模的下限(低于即判基奇异/病态). */
    static final double PIVOT_MIN = Double.parseDouble(System.getProperty("ae2e.pivotMin", "1e-11"));
    /** eta 向量数上限, 超限由调用方重分解. m 为数百时重分解廉价, 取 30 限制因子漂移. */
    static final int MAX_ETA = Integer.getInteger("ae2e.maxEta", 30);

    /** 数值失败异常(基奇异/病态), 由调用方转为 NUMERIC_FAILURE. 携带失败位置与候选行, 供单位列修补恢复. */
    static final class NumericException extends Exception {
        /** 失败列在 factorize 中的位置, -1 表示无定位信息. */
        final int failPos;
        /** 剩余位置中 |work| 最大的矩阵行, 修补单位列的候选行, -1 表示无. */
        final int bestRow;
        /** rowOfPos[failPos..] 的拷贝, 只有这些行的单位列是有效主元, 其余行的单位列已被消元吞没. */
        final int[] candRows;
        ...
        NumericException(String message, int failPos, int bestRow, int[] candRows) {
            super(message);
            this.failPos = failPos;
            this.bestRow = bestRow;
            this.candRows = candRows;
        }
    }
    ...
    /** 对基列集合做稀疏 LU 分解, 同时清空全部 eta. 基列按矩阵行语义给出, 长度必须为 m. */
    void factorize(List<? extends SparseCol> columns) throws NumericException {
        ...
        for (int k = 0; k < this.m; k++) {
            ...
            // 已有 L 列前代, 消去位置 < k 的分量, 元素按位置升序处理所以读写无冲突
            for (int t = 0; t < k; t++) {
                double coeff = work[t];
                if (coeff == 0) {
                    continue;
            ...
            if (pivotPos < 0) {
                // 异常携带失败位置与剩余行映射, 供调用方做单位列修补
                throw new NumericException(
                        "基奇异:列 " + k + " 无主元候选(最大剩余模 " + bestAbs + ")", k,
                        this.rowOfPos[bestPos],
                        java.util.Arrays.copyOfRange(this.rowOfPos, k, this.m));
            }
            if (pivotPos != k) {
                // 交换位置映射、工作向量元素和先前 L 列在 k/pivotPos 处的值, 选主元换行必须同步已存 L 因子
            ...
            // 整行清零: L 前代会写入位置 < k 的元素, 必须全部复位
            java.util.Arrays.fill(work, 0);
        }
    }

    /** 交换稀疏列中两个位置上的值, 选主元换行时用于同步先前 L 列. */
    ...
    /** Forrest–Tomlin 换基更新: 进入列的 FTRAN 结果 α 在 pivotPos 处替换旧基列, 记为 eta 矩阵 E = I + (η − e_p)e_pᵀ. */
    ...
    /** FTRAN: 就地解 B x = b, 输入输出共用同一向量, 调用方视为基序向量即可.
