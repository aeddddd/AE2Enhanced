...
public class ExecutionHarnessTest {
    ...
    public void testSelfRefDuplicationExecution() {
        ...
        assertExecutesCleanly(PlanView.of(env.runLp(mult(stone, 10))),
    ...
    public void testThetaX100AllOrders() {
        ...
        assertExecutesCleanly(PlanView.of(env.runLp(mult(stone, 100))),
    ...
    public void testUserAbcCaseExecution() {
        ...
        assertExecutesCleanly(PlanView.of(env.runLp(mult(stone, 64))),
    ...
    public void testCatalystExecution() {
        ...
        assertExecutesCleanly(PlanView.of(env.runLp(mult(stick, 10))),
    ...
    public void testRationalCycleWithAuxExecution() {
        ...
        assertExecutesCleanly(PlanView.of(env.runLp(mult(stone, 4))),
    ...
    public void testInsufficientInitialExtractionDeadlocks() {
        ...
        PlanView plan = PlanView.of(env.runLp(mult(stone, 100)));
    ...
    /** X3/X10(对照):同一 θ×1000 计划——无配额必死锁;串行预算与默认配额均完成. */
    @Test
    public void testQuotaSchedulerPreventsDeadlock() {
        ...
        PlanView plan = PlanView.of(env.runLp(mult(stone, 1000)));
        assertThat(plan.simulation()).isFalse();
        ...
        // 串行预算:每 tick 仅 1 次推送,仍应推进至完成
        ExecutionHarness.Result serial = ExecutionHarness.execute(plan, network,
                ExecutionHarness.Options.gameDefaults().serial(),
                java.util.Arrays.asList(charge, crush, back));
        assertThat(serial.completed).isTrue();
        assertThat(serial.delivered).isEqualTo(1000);
        // 期末:2 - 999 + 2000 - 1000 = 3(全额生产 [500,499,500])
        assertThat(serial.network.getOrDefault(RecursiveCraftingHelper.canon(stone), 0L)).isEqualTo(3);
