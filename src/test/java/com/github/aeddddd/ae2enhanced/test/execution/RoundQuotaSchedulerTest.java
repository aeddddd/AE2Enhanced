...
public class RoundQuotaSchedulerTest {
    ...
    /** T6+T7:API 一致性——单趟否决集 ≡ 逐次 isPushAllowed;allowance 符号 ≡ isPushAllowed,
     * 闭包外恒不受限(MAX);数值精确性(round 配额余量).GCD 大超轮比恢复断言见 T3. */
    @Test
    public void testApiConsistencyVetoAllowanceIsPushAllowed() {
        ThetaPatterns p = new ThetaPatterns();
        Map<ICraftingPatternDetails, Long> totals = new LinkedHashMap<>();
        totals.put(p.crush, 4L);
        totals.put(p.charge, 4L);
        ...
        // 五种推进状态下:vetoedSet ≡ !isPushAllowed,allowance 符号 ≡ isPushAllowed
        long[][] progresses = { { 4, 4, 4, 7 }, { 3, 4, 4, 7 }, { 4, 3, 2, 5 }, { 0, 3, 4, 0 }, { 0, 0, 0, 0 } };
        ...
        for (long[] prog : progresses) {
            ...
            for (ICraftingPatternDetails pattern : all) {
                ...
                assertThat(allowance > 0)
                        .as("remaining=%s pattern=%s allowance=%s", remaining, pattern, allowance)
                        .isEqualTo(allowed);
            }
